...
class ProductCreateViewTestCase(APITestCase):
    ...
    def test_create_product_with_variations(self):
        product_data = {
            ...
            "active": True,
            "variations": [
                {"name": "Variation 1", "price": 10.0, "active": True},
                {"name": "Variation 2", "price": 15.0, "active": True},
            ],
        }
    ...
    def test_create_product_with_no_variation_creates_a_default_variation_automatically(
        self,
    ):
        product_data = {"name": "Product 1", "active": True}
...
class ProductUpdateViewTestCase(APITestCase):
    def setUp(self):
        ...
        self.variation1 = ProductVariation.objects.create(
            product=self.product, name="Variation 1", price=10.0, active=True
        )
        self.variation2 = ProductVariation.objects.create(
            product=self.product, name="Variation 2", price=15.0, active=True
        )
    ...
    def test_update_product_and_variations(self):
        ...
        updated_data = {
            ...
            "variations": [
                ...
                {"name": "New Variation", "price": 20.0, "active": True},
            ],
        }
...
class ProductDeleteViewTestCase(APITestCase):
    ...
    def test_delete_product_with_active_variations(self):
        ProductVariation.objects.create(
            product=self.product, name="Variation 1", active=True, price=10.0
        )
        url = reverse("admin-product-update-delete", kwargs={"pk": self.product.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
...
class ProductVariationDeleteViewTestCase(APITestCase):
    ...
    def test_delete_product_variation(self):
        url = reverse(
            "admin-product-variation-delete", args=[self.product.id, self.variation.id]
        )
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
